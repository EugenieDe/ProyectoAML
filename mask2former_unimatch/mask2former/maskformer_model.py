...
class MaskFormer(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            "metadata": MetadataCatalog.get(cfg.DATASETS.TRAIN_L[0]),
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            for mask_cls_result, mask_pred_result, input_per_image in zip(
                ...
                height = mask_pred_result.shape[-2]#input_per_image.get("height", image_size[0])
                width = mask_pred_result.shape[-1]#input_per_image.get("width", image_size[1])
                processed_results.append({})

                if self.sem_seg_postprocess_before_inference:
                    mask_pred_result = retry_if_cuda_oom(sem_seg_postprocess)(
                        mask_pred_result, (height, width), height, width
                    )
    ...
    def prepare_targets(self, targets, images, mode=None):
        ...
        for targets_per_image in targets:
            ...
            if mode=="unsupervised":
                gt_masks = targets_per_image.pred_masks
                padded_masks = torch.zeros((gt_masks.shape[0], h_pad, w_pad), dtype=gt_masks.dtype, device=gt_masks.device)
                padded_masks[:, : gt_masks.shape[1], : gt_masks.shape[2]] = gt_masks
                new_targets.append(
                    {
                        "labels": targets_per_image.pred_classes,
                        "masks": padded_masks,
                    }
                )
            else:
        ...
        for targets_per_image in new_targets:
