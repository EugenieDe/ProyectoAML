...
from UniMatch.model.backbone.swin import Transformer
from UniMatch.supervised import evaluate
...
from UniMatch.util.utils import count_params, init_log, AverageMeter

def unimatch(args, labeled_id_path, unlabeled_id_path, save_path):
    ...
    if cfg['criterion']['name'] == 'CELoss':
        ...
        criterion_l = ProbOhemCrossEntropy2d(ignore_index=255, thresh=0.9).to(device) 
    else:
    ...
    trainset_l = EndovisDataset(cfg['dataset'], cfg['data_root'], 'train_l',
                              id_path=labeled_id_path, nsample=len(trainset_u.ids))
